    ...
    class Yolo(torch.nn.Module):
        ...
        def forward(self, x, targets=None):
            if self.training:
                ...
                return self.model(x)
    #yolov5s.pt
    model = yolov5.load(r"C:\Users\vczyp\PycharmProjects\last.pt")
    print(model)

    model = Yolo(model)

    detector = PyTorchYolo(
        model=model, device_type="cuda", input_shape=(3, 640, 640), clip_values=(0, 255), attack_losses=("loss_total",)
    )
...
#################        Example image        #################
"""
mypath = r'C:\Users\vczyp\PycharmProjects\pythonProject\Vehicle\train\images\*.jpg'
import glob
import os
files_img = glob.glob(mypath)

for i in range(0,len(files_img),3):
    response = r'C:\Users\vczyp\PycharmProjects\pythonProject\bicycle.jpg'
    img = np.asarray(Image.open(response).resize((640, 640)))
    ...
    cv2.imwrite(os.path.join(r'C:\Users\vczyp\PycharmProjects\pythonProject\test', str(i) + '.jpg'),img)
...
#plot_image_with_boxes(img=img, boxes=preds[1], pred_cls=preds[0], title="Оригинал")
